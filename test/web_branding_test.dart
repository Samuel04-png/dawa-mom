...
void main() {
  test('web metadata uses DawaMom title and generated compact icons', () {
    final index = File('web/index.html').readAsStringSync();
    final manifest = jsonDecode(File('web/manifest.json').readAsStringSync())
        as Map<String, dynamic>;

    expect(index, contains('<title>DawaMom</title>'));
    expect(index, contains('href="favicon.png"'));
    expect(index, isNot(contains('Group_1_dark.png')));
    expect(manifest['name'], 'DawaMom');
    expect(manifest['short_name'], 'DawaMom');
